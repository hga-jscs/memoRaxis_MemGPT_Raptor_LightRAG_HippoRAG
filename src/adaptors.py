...
class BaseAdaptor(ABC):
    ...
    def _log_evidences(self, evidences: List[Evidence], context: str = "") -> None:
        ...
        for i, e in enumerate(evidences, 1):
            raw_score = e.metadata.get("score", None)
            content_preview = e.content[:80] + "..." if len(e.content) > 80 else e.content

            # 1) score 显示：能转 float 就转；没有就 N/A
            if raw_score is None:
                ...
                try:
                    ...
                    score_str = repr(raw_score)

            # 2) 可选：把 distance / score_source 也一起打出来（调试很有用）
            extras = []
            if "distance" in e.metadata:
                try:
                    extras.append(f"dist={float(e.metadata['distance']):.4f}")
                except Exception:
                    extras.append(f"dist={repr(e.metadata['distance'])}")
            if "score_source" in e.metadata:
                extras.append(f"src={e.metadata['score_source']}")
            extra_str = (", " + ", ".join(extras)) if extras else ""

            self._logger.info("  [%d] (score=%s%s) %s", i, score_str, extra_str, content_preview)
